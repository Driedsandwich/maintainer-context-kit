# Documentation

## Start here

- [Project goal](00_goal.md)
- [v0.1 requirements](01_requirements.md)
- [Local runbook](02_runbook.md)
- [Design overview](07_design.md)
- [Current usage](08_current_usage_note.md)
- [Packet specification](../packet-spec/maintainer-task-packet-v0.1.md)
- [Reusable packet template](../templates/maintainer-task-packet.md)

## Safety and validation evidence

- [Safety language review](15_safety_language_review_note.md)
- [Scope note](16_scope_note.md)
- [Safety hardening phase 1](23_safety_hardening_phase1_note.md)
- [Credential detector coverage note](24_credential_detector_coverage_note.md)
- [CLI value validation](26_cli_value_validation_20260715.md)
- [Template-only comparison](27_template_only_comparison_20260715.md)
- [Repository-aware verification validation](28_repository_aware_verification_validation_20260715.md)

Historical validation notes may describe earlier private-iteration decisions. The public preview posture in the root [README](../README.md), current source, tests, and safety policy is authoritative for this candidate.
